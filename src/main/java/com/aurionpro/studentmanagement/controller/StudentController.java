...
public class StudentController {
    ...
    // Injecting the service interface, not the implementation (loose coupling)
    public StudentController(StudentService studentService) {
        this.studentService = studentService;
    }

    @Operation(summary = "Get all active students", description = "Returns a list of all students whose status is 'active'.")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Successfully retrieved list")
    })
    @GetMapping
    public ResponseEntity<ApiResponse<List<StudentResponseDto>>> getAllActiveStudents() {
        List<StudentResponseDto> students = studentService.getAllActiveStudents();
        ApiResponse<List<StudentResponseDto>> response = new ApiResponse<>("success", "Students fetched successfully", students);
